...
**小米：** 用户觉得有些网页字体太小，先询问是否做放大或手机格式。判断后选择先做 WebView 缩放，因为它只改变当前页面显示比例，不改 User-Agent、不触碰站点数据，风险比移动端模式低。

**小米：** 交互入口放在顶部刷新按钮旁边，而不是任务右键菜单。原因是缩放属于当前页面显示控制，不是任务管理操作；放在右上角更接近浏览器习惯，也不会继续堆右键菜单。

**小米：** 缩放控件采用放大镜图标 + 小气泡。气泡里只放当前比例、加减按钮和重置按钮，避免做成复杂设置项。缩放档位按用户给定列表固定，比例保存到任务状态中，切换任务时由 WebViewManager 应用。

**小米：** 本轮同时检查历史乱码。`package.json` 和 `APP_AUTHOR` 当前已经是 `染泓如梦QAQ`；`releases/README.md` 中文内容重新整理，避免后续发布说明继续带乱码。

### 2026-07-06：界面图标资产替换

**小米：** 用户已经下载好一整套图标，并确认可以开始替换。当前目标不是重新设计 UI，而是把已经露在界面上的文字 / 符号入口换成统一 SVG，降低视觉割裂感。

**小米：** 实施边界放在 renderer 展示层：新增 `src/renderer/assets/icons/` 保存图标，更新 `index.html` 的按钮图片引用，更新 `renderer.js` 中侧边栏和分组状态图标的动态切换，补充 `styles.css` 的图标尺寸和窄栏样式。

**小米：** 保存、返回等暂未出现在界面的图标先不接入。这样可以避免为了“用完图标”而新增按钮或扩大交互范围，保持 ChatHub 作为自用小工具的简洁边界。

**小米：** 本轮不触碰 WebViewPool、TaskManager、登录状态、任务切换和打包配置，风险集中在纯 UI 图标显示。

### 2026-07-06：发布 v1.0.0 正式版

**小米：** 用户确认 ChatHub 可以作为第一个正式版发布，版本号指定为 `1.0.0`。这次发布的意义不是继续扩展功能，而是把当前已经打磨稳定的自用小工具定格为正式版。

**小米：** 发布边界保持收束：纳入当前页面缩放、统一 SVG 图标、输入草稿修复、WebViewPool、分组和安装卸载链路等已有稳定能力；不新增新的业务系统。

**小米：** README 和 Release 说明需要明确维护策略：后续不再默认做功能性更新，只处理 bug、网页兼容、安装卸载和必要性能细节。这样可以防止小工具继续膨胀，保持“怕麻烦”的初衷。

**小米：** 发布仍按既有规则执行：完整打包、仓库内安装包副本同步、提交 GitHub、创建全新 `v1.0.0` tag 和 GitHub Release，不覆盖任何历史版本。

### 2026-07-09：主窗口分屏对照模式

**小米：** 用户提出有时需要两个页面同时对照。最初讨论过拖出独立窗口，但真正需求是“两个页面同时显示”，因此选择主窗口内左右分屏，而不是新增多窗口系统。

**小米：** 设计边界：左侧仍是当前任务，右侧是临时对照任务。入口放在任务右键菜单“在右侧打开”，避免新增复杂拖拽规则，也符合当前 ChatHub 右键管理任务的交互习惯。

**小米：** 用户进一步要求分屏后可以选择关闭左边还是右边，并且关闭后保留另一个页面。因此顶部提供“关闭左侧 / 关闭右侧”：关闭右侧时左侧保持为主页面；关闭左侧时右侧接管为主页面并恢复单栏。

**小米：** 实现边界放在 renderer 和 WebViewManager：新增 `comparisonTaskId` 作为右侧槽位，WebViewPool 仍然统一管理 WebView 生命周期。LRU 淘汰逻辑排除当前左 / 右两个显示中的 WebView，不清理 cookies、localStorage、IndexedDB，也不改变 Electron 持久化分区。

### 2026-07-09：分屏拖放交互补充

**小米：** 用户希望分屏不只靠右键，也能通过拖动直接触发，并且左右两页可以拖动调换位置。这个需求仍然属于已有分屏体验打磨，不新增多窗口或新任务系统。

**小米：** 实现选择复用左侧任务列表已有的拖拽数据：拖任务到工作区右半边即打开右侧对照，拖到左半边即作为左侧主页面。分屏状态下，如果把当前左侧任务拖到右半边，或把右侧对照任务拖到左半边，就交换 `currentTaskId` 和 `comparisonTaskId`。

**小米：** WebViewManager 的交换只改可见槽位，不调用 `loadURL()`，因此不会重载页面，也不会影响登录状态。视觉上只加浅色落点提示，不增加说明文字，避免把小工具界面变复杂。

### 2026-07-09：修复拖拽分屏难触发

**小米：** 用户反馈拖拽分屏很难触发，左右互换也几乎没成功。判断原因是 Electron `webview` 会像独立浏览器区域一样吞掉拖放事件，导致把工作区本身作为 drop target 不稳定。

**小米：** 修复策略是不改变分屏业务逻辑，只改变拖拽承接层：拖动任务开始时，在 WebView 上方显示一层 `split-drop-overlay`，左右各一块投放区。drop 事件落在覆盖层上，再转发给 Controller 执行左侧打开、右侧打开或左右互换。

**小米：** 这样可以避免 WebView 抢事件，同时不需要新增说明文字或复杂手势。拖拽结束、取消、窗口失焦、Esc、drop 后都会隐藏覆盖层，防止界面残留。

### 2026-07-09：分屏页面内工具条

**小米：** 用户提出按钮应该和页面放在同一层：默认一个大页面面板右上角有刷新 / 缩放，不显示关闭；分屏时两个小页面面板各自右上角有刷新、缩放和关闭。

**小米：** 这比把所有控制放在顶部更符合分屏心智，因为关闭按钮属于具体页面，而不是整个窗口。实现上把 topbar 中的刷新 / 缩放 / 关闭迁移到 `webview-frame` 内部的左右 `pane-toolbar`，单页只显示左工具条，分屏时显示左右两个工具条。

**小米：** 右侧工具条需要直接作用于右侧任务，所以 Controller 新增按 side 获取 task 的逻辑，WebViewManager 新增按 taskId 刷新 WebView 的能力。这样右侧刷新 / 缩放不需要先把右侧切成主任务，也不会触发页面重载以外的额外状态切换。

### 2026-07-09：页面工具条遮挡修复

**小米：** 用户反馈单页也显示关闭按钮，且页面内工具条遮挡网页内容。判断需要把工具条从“悬浮覆盖”改成“页面面板预留区域”。

**小米：** 样式修复分两层：非分屏状态下通过 `.webview-frame:not(.split-mode) .pane-close-button` 强制隐藏关闭按钮；同时给 `webview-frame` 增加顶部 padding，让 WebView 从工具条下方开始显示。

**小米：** 顶部当前任务区同步压缩，从 74px 降到 56px，标题字号略小。这样页面面板内部多出的工具条高度不会让可视区域损失太明显。

### 2026-07-09：页面矩形边界视觉调整

**小米：** 用户希望按钮不要像悬浮层，而是和页面矩形在同一层，并且分屏时能明显看出左右两个矩形。截图里的红框只是示意，不能真的使用红色边框。

**小米：** 因此把 `pane-toolbar` 从悬浮白色气泡改为页面矩形的头部条：工具条是第一行，WebView 是第二行主体。单页是一张卡片，分屏是两张并列卡片，边界用现有淡色 border 和浅色头部区表达。

**小米：** 拖放覆盖层同步下移到工具条下方，避免拖拽提示盖住页面按钮。这样工具条、页面主体、拖拽提示三者层级更清楚。

### 2026-07-09：工具栏拖拽交换分屏位置

**小米：** 用户希望左右交换的触发位置放在页面上方工具栏，直接交换两个矩形的位置。这个交互比拖网页主体更清楚，因为工具栏已经成为页面矩形的头部，也不会干扰 WebView 内部网页操作。

**小米：** 实现上给左右 `pane-toolbar` 增加独立拖拽类型 `text/chathub-pane`。拖左工具栏到右工具栏，或反过来，调用现有 `swapSplitSides()`，只交换 `currentTaskId` 和 `comparisonTaskId`，不调用 `loadURL()`。

**小米：** 工具栏里的按钮和缩放弹窗不启动拖拽，避免点击刷新、缩放或关闭时误触交换。拖拽目标只做淡色高亮，不增加额外说明文字。

### 2026-07-09：侧边栏分组图标方向修复

**小米：** 用户确认分屏交互已经没问题，本轮只处理左侧侧边栏分组展开 / 收起图标用反的问题。

**小米：** 修改范围控制在 renderer 展示层，将分组折叠状态映射为右向图标、展开状态映射为下向图标；不触碰分屏、WebViewPool、任务管理或登录状态。

### 2026-07-09：发布 v1.0.1 维护版

**小米：** 用户确认分屏已经没问题，并要求提交版本 `1.0.1`。本轮作为维护版收口，不再扩展新功能范围。

**小米：** 版本内容包含分屏对照模式打磨、页面内工具条、拖拽分屏、工具栏拖拽交换左右位置，以及左侧分组展开 / 收起图标方向修复。提交前需要完整构建确认 exe 元数据版本同步为 `1.0.1`。

### 2026-07-09：覆盖安装升级流程修复

**小米：** 用户反馈升级版本覆盖安装时会直接执行卸载，卸载完成后需要再次运行安装包才会重新安装。判断问题在 NSIS 升级卸载链路，而不是 Electron 业务代码。

**小米：** electron-builder 升级时会调用旧卸载器并传入 `--updated`，但当前自定义 `customUnInstall` 没判断 `${isUpdated}`，导致升级卸载也执行完整安装目录清理和延迟删除。

**小米：** 只修新卸载器还不够，因为从 v1.0.1 升级时先运行的是旧 v1.0.1 卸载器。新安装器需要在旧卸载器返回后等待旧延迟清理脚本结束，再释放新文件，避免新文件被旧清理脚本删除。

**小米：** 本轮修复限制在 `build/installer.nsh` 和版本 / 文档记录：升级模式跳过自定义完整清理，同时新增 `customUnInstallCheck` 兼容旧延迟清理；不修改 WebView、任务、分屏或登录状态逻辑。

### 2026-07-09：完全退出后恢复默认页面

**小米：** 用户希望完全退出 ChatHub 后重新启动，所有 WebView 任务都回到默认入口，但登录状态保留。这个需求不是清账号，而是清页面运行态和缓存。

**小米：** 当前回到旧页面的主要原因是 `saveSnapshot()` 会把 WebView 当前 URL 保存到任务 `url`。下次启动时任务使用这个 URL，自然会打开上次退出前的会话页。

**小米：** 最小风险修法是启动时把 `task.url` 重置回 `task.initialUrl`，同时清空 `inputDraft/messages/scroll`；退出前只调用 runtime session 的 `clearCache()`，不调用 `clearStorageData()`，因此不碰 cookies、localStorage、sessionStorage 或 IndexedDB。

**小米：** 用户实测覆盖安装仍然只卸载，说明旧卸载器返回状态也会影响新安装器继续执行。升级模式下改为：如果旧卸载器返回非 0，但旧 `ChatHub.exe` 已不存在，则视为旧程序文件已经移除，继续安装；如果旧 exe 还在，才视为真正失败并中止。

### 2026-07-09：发布 v1.1.0 维护版

**小米：** 用户确认覆盖安装现在可以继续自动重新安装，只是过程表现为先卸载再安装。这个行为符合 NSIS / electron-builder 的升级安装机制，因此本轮按 `1.1.0` 维护版收口。

**小米：** 发布前清理本轮测试残留，只删除明确的测试安装目录 `D:\codex\ChatHub_UpgradeSmoke` 和测试安装包 `dist\ChatHub-Setup-v1.0.1-test.exe`，不碰正常构建产物、用户数据或登录状态。
