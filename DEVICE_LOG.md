...
### 2026-07-04：右键任务从“重命名”改为“修改网页”

**小米：** 用户提到一个一直忘记修的小交互：已经添加的页面右键里不应该只有“重命名”，而是要能同时修改名称和链接。

**小米：** 这个需求属于维护期的小修，不适合新增复杂编辑器。最小风险方案是复用现有“添加网页”弹窗，让它根据入口切换为“添加”或“修改”模式。

**小米：** 修改名称只更新 TaskManager 中的 `title/name`；修改链接时需要同步更新 `url/initialUrl/origin`，并清掉该任务自己的临时输入草稿和滚动快照，避免旧站点状态恢复到新站点。

**小米：** WebViewPool 不需要改核心逻辑。若链接发生变化，只移除该任务旧的 WebView 缓存实例；下次打开或当前重新加载时沿用现有 `loadTask()` 创建新实例。这个动作不清理 Electron session，所以不会主动影响登录状态。

### 2026-07-04：发布 v0.6.6 维护版

**小米：** 用户要求把右键修改网页修复打包提交，并同步发布 Release，版本号指定为 `0.6.6`。这属于稳定维护期的小版本修复，不改变功能边界。
