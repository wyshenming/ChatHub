...
- 完整 NSIS 安装包已构建，并已创建全新 annotated tag `v1.5.0` 和双语 GitHub Release：`https://github.com/wyshenming/ChatHub/releases/tag/v1.5.0`。
...
- 将 `package.json`、`package-lock.json`、`src/preload.js`、README 与发布说明统一升级到 `1.2.7`。
- 在 `releases/README.md` 中新增 v1.2.7 中英双语维护版说明，保留历史版本记录。
- 收口右侧分屏状态、自动恢复上限、刷新反馈、弹窗中线层级和删除分屏任务后的状态同步修复。
- `npm run dist` 完整打包通过，重新生成 x64 unpacked 应用与 NSIS 安装包。
- `dist/ChatHub.exe` 与 `dist/win-unpacked/ChatHub.exe` 的产品版本和文件版本均为 `1.2.7`，PE machine 为 `0x8664`。
- `dist/ChatHub.exe --quit-for-uninstall` 无界面退出烟测通过，退出码为 0。
...
- WebView 状态通知只允许左侧主页面触发，右侧分屏完成、失败或刷新后的状态回调被过滤，工具栏会一直停在“正在加载”。
- 分屏分隔条与弹窗内容使用相同层级且高于遮罩，打开设置、添加网页等模态页面后仍会显示在遮罩上方。
- WebView 自动重建后的 `about:blank` 初始化完成事件会错误清零恢复次数，网络持续异常时会绕过原有的一次上限，每隔约 30 秒反复重建。
- 删除右侧分屏任务时，TaskManager 会无条件把主任务改成列表第一项，而 WebViewManager 仍保留原左侧主页面，导致 Controller、任务列表与实际 WebView 的主任务 ID 不一致；下一次拖入分屏可能被错误判定为已在左侧。

### 本轮已经完成

- WebView 加载、就绪和失败通知改为覆盖当前可见的左侧主页面与右侧分屏页面，不影响后台缓存页面。
- 模态页面打开时隐藏分屏分隔条并禁用其指针事件，同时保留原分屏网格宽度，关闭弹窗后布局不跳动。
- 自动恢复明确限制为 1 次；`about:blank` 不再清零次数，第二次超时会停止重建并显示加载失败。真实网页成功加载或用户手动刷新后可以重新获得一次恢复机会。
- 点击左右任一刷新按钮后，图标立即旋转 700 毫秒、按钮暂时禁用且对应状态显示“正在刷新”；真实加载事件随后继续更新“正在加载”“已就绪”或“加载失败”。
- 删除非主任务时保留当前主任务；删除完成后先同步分屏提升或新主页面加载，再统一渲染任务与分屏布局，避免首次拖入页面不显示。

### 验证结果

- 主进程、preload、全部 renderer JavaScript 和打包脚本共 11 个文件通过 `node --check`。
- 状态通知回归脚本确认：左侧与右侧可见页面会分别触发回调，后台缓存页面不会误触发。
- 自动恢复上限回归脚本确认：`about:blank` 保留恢复次数，真实网页成功后清零，达到 1 次上限后的下一次超时会停止重建并报告失败。
- 分屏删除回归脚本确认：删除右侧比较任务后保留左侧主任务，Controller、TaskManager 与最终渲染状态一致。
- 静态检查确认 `modal-open` 状态会隐藏分隔条并禁用其指针事件。
- `npm run dist:dir` 已生成新的 x64 unpacked 应用；首次复制因旧版 ChatHub 正在运行并锁定 EXE 而中断，使用项目自带 `--quit-for-uninstall` 正常退出后完成运行时复制。
- 新版 `dist/ChatHub.exe --quit-for-uninstall` 无界面启动 / 退出烟测通过，退出码为 0。

## 2026-07-21 依赖安全基线记录

### 本轮已经完成

- 新增 `DEPENDENCY_SECURITY.md`，单独记录 `npm audit` 报告的 7 个高危、1 个严重依赖告警。
- 在 `README.md` 中新增中英双语依赖安全状态说明，并链接独立安全记录。
- 区分 Electron 运行时风险与构建工具链风险，并记录个人自用、可信构建来源下的实际影响判断。
- 明确当前不执行强制升级，后续在独立升级分支中处理 Electron 与 electron-builder。
- 记录未来升级分支的 WebView、登录状态、分屏、打包、安装和卸载验收范围。

### 验证结果

- `npm audit --json` 已重新确认当前 8 个受影响依赖包。
- `npm audit --omit=dev --json` 为 0；文档已说明 Electron 虽声明为开发依赖，但运行时会进入最终应用。
- 本轮只修改文档，不修改依赖、业务代码或构建产物。
...
- 分屏仍默认使用 50% / 50% 布局。
- 在左右页面之间增加可拖动分隔条，拖动时实时调整两侧宽度。
- 左侧比例限制为 25% 至 75%，因此任一侧最小 25%、最大 75%，不会被完全压缩。
- 拖动期间临时禁止 WebView 抢占指针事件，保证鼠标跨过网页内容时仍能连续调整。
- 分隔条支持键盘左右方向键微调，`Shift + 方向键` 每次调整 5%，`Home` / `End` 直接切换到 25% / 75%。
- 分屏比例仅在当前应用运行期间保留，重新启动后恢复默认 50% / 50%。
...
- `node --check` 已通过：主进程、preload 和全部 renderer JavaScript 文件。
- 分隔条 DOM、事件绑定和三列 Grid 布局的静态接线检查通过。
- `git diff --check` 通过，仅有 CRLF 提示。
- `npm run dist:dir` 成功，已生成新的 x64 unpacked 应用。
- `dist/ChatHub.exe --quit-for-uninstall` 无界面启动 / 退出烟测通过，退出码为 0，未残留 ChatHub 进程。
- 待人工验证：进入分屏后拖动中线，确认两侧可在 25% / 75% 与 75% / 25% 之间连续变化，并且页面交互与登录状态正常。

## 2026-07-17 v1.2.5 发布
...
- 已完成：创建发布提交 `ea7e64f`，并将 `main` 推送到 GitHub。
- 已完成：创建并推送全新 annotated tag `v1.2.5`，未覆盖任何历史 tag / Release。
- 已完成：创建 GitHub Release `v1.2.5` 并上传 `ChatHub-Setup-x64.exe`。
...
- 新增主窗口内左右分屏对照能力。
- 任务右键菜单新增“在右侧打开”，用于把某个任务作为右侧对照页显示。
- 分屏时左侧保留当前任务，右侧显示对照任务；两个页面同时显示、同时保持各自 WebView。
- 顶部新增“关闭左侧”和“关闭右侧”按钮：
  - 关闭左侧：右侧对照页接管为主页面并恢复单栏。
  - 关闭右侧：左侧当前页保留并恢复单栏。
- WebViewPool 增加 `comparisonTaskId` 槽位，LRU 不淘汰当前显示的左 / 右两个 WebView。
- 支持将左侧任务直接拖到主工作区：
  - 拖到右半边：作为右侧对照页打开。
  - 拖到左半边：作为左侧主页面打开。
  - 分屏时拖动左 / 右任务到对侧可完成左右互换，不重新加载页面。
- 修复初版拖拽分屏难触发的问题：拖动任务时在 WebView 上方显示专用投放覆盖层，避免 Electron WebView 吞掉拖放事件。
- 修复拖动已在分屏中的任务到自身所在侧后布局错乱的问题：左右 WebView 强制固定在同一 grid 行，且拖到自身所在侧只刷新槽位、不触发交换。
- 将刷新、缩放、关闭从顶部工具区移动到页面面板右上角：
  - 单页时页面右上角显示刷新 / 缩放，不显示关闭。
  - 分屏时左右两个页面各自显示刷新 / 缩放 / 关闭。
  - 右侧缩放和刷新直接作用于右侧任务，不强制切换主任务。
- 修复页面工具条遮挡网页内容的问题：页面面板顶部预留工具条高度，WebView 从工具条下方开始显示。
- 收紧单页关闭按钮隐藏规则：非分屏状态下 `.pane-close-button` 强制不显示。
- 缩小顶部当前任务区域高度和标题字号，为页面面板留出更多空间。
- 将页面工具条从悬浮样式改为每个页面矩形的头部条，WebView 作为矩形主体显示，分屏时左右两个矩形边界更清楚。
- 分屏时支持拖动页面顶部工具栏到另一侧工具栏，直接交换左右两个页面矩形的位置，不重新加载 WebView。
...
- 待验证：右键任务选择“在右侧打开”后左右两页同时显示。
- 待验证：关闭左侧后右侧页面保留并变成单栏主页面。
- 待验证：关闭右侧后左侧页面保留并变成单栏主页面。
- 待验证：拖动任务到工作区左 / 右半边可以打开或调换分屏位置。
- 待验证：覆盖层出现后拖放触发率是否明显改善。
- 待验证：当前左侧为 ChatGPT、右侧为 Gemini 时，再拖 ChatGPT 到左侧不会导致页面错位。
- 待验证：单页和分屏时页面内工具条位置、层级和点击行为正常。
- 待验证：单页状态不再显示关闭按钮，工具条不遮挡网页内容。
- 待验证：分屏时左右两个页面矩形有清晰边界，且不使用示意图中的红色边框。
- 待验证：拖动左 / 右页面工具栏到对侧工具栏可以稳定互换位置。
- 待验证：分屏不清理登录状态，不影响已有 WebViewPool 缓存。

## 2026-07-09 侧边栏分组图标方向修复

### 本轮已经完成

- 修复左侧侧边栏分组展开 / 收起图标用反的问题。
- 折叠状态使用右向图标，展开状态使用下向图标。
- 未修改分屏逻辑、WebView 生命周期或任务状态逻辑。
...
- 已验证：`node --check src\renderer\renderer.js` 通过。
- 已验证：`git diff --check` 仅有 CRLF 提示。
- 待验证：启动应用确认分组展开 / 收起图标方向符合预期。

## 2026-07-09 发布 v1.0.1 维护版

### 本轮已经完成

- 按用户要求将版本号升级到 `1.0.1`。
- 纳入分屏对照模式相关打磨：左右页面同时显示、页面内工具条、关闭左 / 右侧、拖拽分屏、工具栏拖拽交换左右位置。
- 纳入左侧侧边栏分组展开 / 收起图标方向修复。
- 更新 `README.md` 与 `releases/README.md` 的当前版本和维护版说明。
...
- 已验证：`git diff --check` 通过，仅有 CRLF 提示。
- 已验证：`npm run dist` 完整打包通过。
- 已验证：`dist\ChatHub.exe` 元数据显示 `FileVersion=1.0.1`、`ProductVersion=1.0.1`。
- 已完成：复制安装包到 `releases/ChatHub-Setup-x64.exe`。
- 新安装包 SHA256：`28BC493C130AC3DE43AD9A69E90DF40FC31407A7492D65C55758E79C4FFED7AE`。
- 已完成：提交并推送 GitHub 仓库。

## 2026-07-11：网页编辑弹窗关闭行为

- 已修复：添加网页与修改网页共用的弹窗不再因点击弹窗外的应用区域而关闭。
- 保留：右上角关闭按钮与“取消”按钮仍可关闭弹窗。
- 未构建、未提交：本轮仅完成 renderer 交互修复，待用户集中测试或发布时再打包。
