...
- 已完成：版本统一升级到 `1.5.0`，合并依赖安全升级、WebView 资源诊断日志和深色模式。
- 已完成：完整构建 NSIS 安装包，创建全新 `v1.5.0` tag 与双语 GitHub Release。
- 后续独立维护：处理全量 `npm audit` 中 5 个开发 / 构建工具链高危告警；运行时 `npm audit --omit=dev` 当前为 0。
- 已完成：README 中英双语记录 5 个开发 / 构建依赖组的影响与日常运行边界。
- 已完成：x64 安装包构建、版本资源检查与无界面启动/退出烟测；安装包 SHA256 为 `884F82DEDA88A1AA58DC261C271A5B48A2500BD34D0E638A170E5E3DFD69E0A4`。
